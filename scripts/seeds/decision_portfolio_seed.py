...
DECISION_POINTS = [
    {
        ...
        "contract": {
            ...
            "consumes": ["problem_intent", "want_type"],
    ...
    },
    {
        ...
        "contract": {
            ...
            "consumes": ["contract_diff"],
            "produces": ["bridged_primitive"],
        },
    ...
    {
        "decision_id": "decision:orchestration.ordering_strategy",
        "title": "Primitive ordering / wiring strategy",
        "question": "How to order and wire primitives into a solution - deterministic compile, framework fill, or LLM proposal?",
        "regime": "runtime",
        "context_signature": ["has_framework"],
        "contract": {
            "input": "a problem as (have ports -> wanted port), + available primitives/frameworks",
            "output": "a validated ordered wiring (PlanLock) of primitives",
            "win_definition": "the produced order type-checks (validates) and, ideally, is minimal",
            "consumes": ["candidate_set", "have", "want"],
            "produces": ["validated_order"],
        },
        "selection_policy": "deterministic_tier",
        "default_path": "path:order.deterministic_compile",
        "execute_all_when_cheap": True,
    },
    {
        "decision_id": "decision:solve.execution_target",
        "title": "Execution target for a validated plan",
        "question": "Where to run a validated plan - local, sandbox, or a remote service - by effect + risk?",
        "regime": "runtime",
        "context_signature": ["effect_profile"],
        "contract": {
            "input": "a validated ordered plan + its effect/risk profile",
            "output": "an execution result + receipt trail",
            "win_definition": "the plan runs to completion with receipts at least cost / risk",
            "consumes": ["validated_order", "effect_profile"],
            "produces": ["execution_result"],
        },
        "selection_policy": "deterministic_tier",
        "default_path": "path:exec.local_python",
        "execute_all_when_cheap": False,
    },
    {
        "decision_id": "decision:ingest.acquire_strategy",
        "title": "Source acquisition strategy",
        "question": "How to acquire a source snapshot - cached, structured API, or AST parse - cheapest rung that yields it?",
        "regime": "runtime",
        "context_signature": ["has_cached_snapshot", "has_structured_api"],
        "contract": {
            "input": "a source descriptor (+ what access rungs are available)",
            "output": "a source snapshot + acquisition receipt (retrieved_mode disclosed)",
            "win_definition": "snapshot obtained at the least escalation with a valid receipt",
            "consumes": ["source_descriptor"],
            "produces": ["source_snapshot"],
        },
        "selection_policy": "deterministic_tier",
        "default_path": "path:ingest.cached_snapshot",
        "execute_all_when_cheap": False,
    },
    {
        ...
        "contract": {
            ...
            "consumes": ["proof_stages"],
            "produces": ["stage_order"],
        },
...
EXECUTION_PATHS = [
    ...
    {
        "path_id": "path:order.deterministic_compile",
        "decision_id": "decision:orchestration.ordering_strategy",
        "title": "Deterministic route compile",
        "method": "forward-chaining route compiler produces a minimal topological order; zero model calls",
        "applicability": {"requires_keys": [], "conditions": []},
        "cost_model": {"tokens": 0, "latency_ms": 5, "side_effect_risk": "none", "build_effort": "low"},
        "reversibility": "reversible", "preference_rank": 0, "deterministic": True,
        "expected_receipts": ["order_valid", "step_count"],
    },
    {
        "path_id": "path:order.framework_fill",
        "decision_id": "decision:orchestration.ordering_strategy",
        "title": "Framework fill",
        "method": "bind a reusable solution framework's slots to concrete producers in its declared wiring order",
        "applicability": {"requires_keys": ["has_framework"],
                          "conditions": [{"key": "has_framework", "op": "truthy"}]},
        "cost_model": {"tokens": 0, "latency_ms": 8, "side_effect_risk": "none", "build_effort": "low"},
        "reversibility": "reversible", "preference_rank": 1, "deterministic": True,
        "expected_receipts": ["order_valid", "framework_id"],
    },
    {
        "path_id": "path:order.llm_propose",
        "decision_id": "decision:orchestration.ordering_strategy",
        "title": "LLM-proposed order (validated + repaired)",
        "method": "a model proposes an order; the deterministic validator disposes and repair() fixes an invalid proposal",
        "applicability": {"requires_keys": [], "conditions": []},
        "cost_model": {"tokens": 400, "latency_ms": 700, "side_effect_risk": "low", "build_effort": "low"},
        "reversibility": "reversible", "preference_rank": 2, "deterministic": False,
        "expected_receipts": ["proposal_valid", "repaired", "order_valid"],
    },
    # ---- decision:solve.execution_target ----
    {
        "path_id": "path:exec.local_python",
        "decision_id": "decision:solve.execution_target",
        "title": "Local pure-python execution",
        "method": "run the plan in-process via the route runtime; cheapest, for pure/low-risk effects",
        "applicability": {"requires_keys": [], "conditions": []},
        "cost_model": {"tokens": 0, "latency_ms": 10, "side_effect_risk": "none", "build_effort": "low"},
        "reversibility": "reversible", "preference_rank": 0, "deterministic": True,
        "expected_receipts": ["execution_receipt"],
    },
    {
        "path_id": "path:exec.sandbox_runner",
        "decision_id": "decision:solve.execution_target",
        "title": "Sandboxed runner",
        "method": "run the plan in a resource-bounded sandbox when effects touch files/network/code",
        "applicability": {"requires_keys": ["effect_profile"],
                          "conditions": [{"key": "effect_profile", "op": "ne", "value": "pure"}]},
        "cost_model": {"tokens": 0, "latency_ms": 200, "side_effect_risk": "low", "build_effort": "medium"},
        "reversibility": "reversible", "preference_rank": 1, "deterministic": True,
        "expected_receipts": ["execution_receipt", "sandbox_isolation"],
    },
    {
        "path_id": "path:exec.remote_service",
        "decision_id": "decision:solve.execution_target",
        "title": "Remote service execution",
        "method": "dispatch the plan to a remote worker/service for scale or special hardware",
        "applicability": {"requires_keys": ["effect_profile"],
                          "conditions": [{"key": "effect_profile", "op": "eq", "value": "scale"}]},
        "cost_model": {"tokens": 0, "latency_ms": 800, "side_effect_risk": "medium", "build_effort": "high"},
        "reversibility": "reversible", "preference_rank": 2, "deterministic": True,
        "expected_receipts": ["execution_receipt", "remote_dispatch"],
    },
    # ---- decision:ingest.acquire_strategy ----
    {
        "path_id": "path:ingest.cached_snapshot",
        "decision_id": "decision:ingest.acquire_strategy",
        "title": "Cached snapshot",
        "method": "read a previously captured source snapshot from cache; cheapest, no network",
        "applicability": {"requires_keys": [], "conditions": []},
        "cost_model": {"tokens": 0, "latency_ms": 2, "side_effect_risk": "none", "build_effort": "low"},
        "reversibility": "reversible", "preference_rank": 0, "deterministic": True,
        "expected_receipts": ["source_digest", "retrieved_mode"],
    },
    {
        "path_id": "path:ingest.structured_api",
        "decision_id": "decision:ingest.acquire_strategy",
        "title": "Structured API / spec",
        "method": "pull an OpenAPI/MCP/registry spec - typed signatures without HTML scraping",
        "applicability": {"requires_keys": ["has_structured_api"],
                          "conditions": [{"key": "has_structured_api", "op": "truthy"}]},
        "cost_model": {"tokens": 0, "latency_ms": 120, "side_effect_risk": "low", "build_effort": "low"},
        "reversibility": "reversible", "preference_rank": 1, "deterministic": True,
        "expected_receipts": ["source_digest", "retrieved_mode"],
    },
    {
        "path_id": "path:ingest.ast_parse",
        "decision_id": "decision:ingest.acquire_strategy",
        "title": "AST parse of source",
        "method": "clone + tree-sitter/AST parse a repo to extract typed signatures deterministically",
        "applicability": {"requires_keys": [], "conditions": []},
        "cost_model": {"tokens": 0, "latency_ms": 800, "side_effect_risk": "low", "build_effort": "medium"},
        "reversibility": "reversible", "preference_rank": 2, "deterministic": True,
        "expected_receipts": ["source_digest", "symbols_seen"],
    },
